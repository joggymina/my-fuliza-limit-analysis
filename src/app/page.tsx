...
export default function FulizaBoostPage() {
  ...
  const [isSuccessOpen, setSuccessOpen] = React.useState(false); // New state for success screen
  const [idNumber, setIdNumber] = React.useState('');
  const [phoneNumber, setPhoneNumber] = React.useState('');
  const [isLoading, setLoading] = React.useState(false);
  const [errorMsg, setErrorMsg] = React.useState<string | null>(null);

  // Safe initial state for recent increase
  ...
  // Reset to main page when user clicks "Return to Dashboard"
            ...
            {isSuccessOpen && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-b from-[#e6fff2] to-[#f0fff5] px-4">
                <div className="w-full max-w-sm rounded-2xl bg-white p-8 shadow-2xl ring-1 ring-slate-200 text-center">
                  <h1 className="text-3xl font-bold text-[#0cc45f] mb-2">Success!</h1>
